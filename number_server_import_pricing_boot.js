...
async function fiveGuest(path){try{return (await axios.get(FIVE_BASE+path,{timeout:20000})).data}catch{return null}}
async function vak(action,params={}){try{return (await axios.get(VAK_BASE,{params:{action,api_key:VAK_KEY,...params},timeout:20000})).data}catch{return null}}
...
async function renderCountries(bot,q,server,page=1){const c=await cfg(server),arr=c.countries.slice((page-1)*5,page*5),total=Math.max(1,Math.ceil(c.countries.length/5));const rows=arr.map(x=>[Markup.button.callback(`🌍 ${x.name}`,`nsi_country_${server}_${enc(x.id)}`)]);const nav=[];if(page>1)nav.push(Markup.button.callback('⬅',`nsi_c_${server}_${page-1}`));if(page<total)nav.push(Markup.button.callback('➡',`nsi_c_${server}_${page+1}`));if(nav.length)rows.push(nav);rows.push([Markup.button.callback('⬅ Servers','ns_user_menu')]);return bot.telegram.editMessageText(q.message.chat.id,q.message.message_id,undefined,`🌍 Select Country\n\nPage ${page}/${total}`,Markup.inlineKeyboard(rows));}
async function importFive(){if(!FIVE_TOKEN)throw Error('FIVESIM_API_KEY is not configured');const c=await cfg('5sim');const oldPrice=new Map(c.services.map(s=>[String(s.id),Number(s.price||0)]));const d=await fiveGuest('/guest/countries');let countries=[];if(Array.isArray(d))countries=d;else if(d&&typeof d==='object')countries=Object.entries(d).map(([id,v])=>({id,name:v?.name||v?.title||id,code:v?.iso||v?.code||''}));if(!countries.length){const p=await fiveGuest('/guest/prices');if(p&&typeof p==='object')countries=Object.keys(p).map(id=>({id,name:id,code:''}))}const services=new Map();for(const co of countries){const p=await fiveGuest(`/guest/prices?country=${enc(co.id)}`);const block=p?.[co.id];if(block&&typeof block==='object')Object.keys(block).forEach(id=>{if(!services.has(id))services.set(id,{id,name:id,price:oldPrice.get(id)||0})})}c.countries=countries.map(x=>({id:String(x.id).toLowerCase(),name:String(x.name||x.id),code:x.code||''}));c.services=[...services.values()];await c.save();return {countries:c.countries.length,services:c.services.length};}
