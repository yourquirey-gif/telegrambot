...
function withMarkup(base,c){const p=Number(base||0),m=Number(c?.markup??c?.profitPercent??c?.profit_percentage??0);if(!Number.isFinite(p)||p<=0)return 0;return Math.ceil(p*(1+(Number.isFinite(m)&&m>0?m:0)/100));}
function recursiveCount(value,depth=0){
  ...
  let total=0;
  for(const k of ['count','stock','qty','quantity','available','available_count']){const n=Number(value[k]);if(Number.isFinite(n)&&n>=0)total+=n;}
  for(const [k,v] of Object.entries(value)){if(['count','stock','qty','quantity','available','available_count'].includes(k))continue;if(v&&typeof v==='object')total+=recursiveCount(v,depth+1);}
  return total;
}
async function fiveData(country,service){return fiveGuest(`/guest/prices?country=${enc(country)}&product=${enc(service)}`)}
async function fiveStock(country,service,operator){const data=await fiveData(country,service);const block=data?.[country]?.[service];if(!block)return 0;if(operator&&operator!=='any'&&block[operator])return recursiveCount(block[operator]);return recursiveCount(block)}
...
function findBlock(data,country,service){
  ...
  let found=null;
  const walk=(o,d=0)=>{if(found||!o||typeof o!=='object'||d>7)return;if(o[country]&&typeof o[country]==='object'&&o[country][service]){found=o[country][service];return}if(o[service]&&typeof o[service]==='object'&&o[service][country]){found=o[service][country];return}for(const v of Object.values(o))if(v&&typeof v==='object')walk(v,d+1)};
...
async function priceFor(server,country,s,c){
  ...
  if(price>0)return withMarkup(price,c);
  if(server==='5sim'){
    ...
    return withMarkup(Math.ceil(Math.min(...nums)*rate),c);
  }
  ...
  return vals.length?withMarkup(Math.ceil(Math.min(...vals)*rate),c):0;
}
async function renderServices(bot,q,server,countryId,p=1){
  ...
  for(const s of arr){
    const stock=server==='5sim'?await fiveStock(country.id,s.id,(c.operators.find(o=>String(o.country)===String(country.id)&&String(o.service).toLowerCase()===String(s.id).toLowerCase())||{}).operator||'any'):await vakStock(country.id,s.id);
  ...
  const nav=[];if(p>1)nav.push(Markup.button.callback('⬅',`ns_services_${server}_${enc(country.id)}_${p-1}`));if(p<total)nav.push(Markup.button.callback('➡',`ns_services_${server}_${enc(country.id)}_${p+1}`));if(nav.length)rows.push(nav);rows.push([Markup.button.callback('⬅ Countries',`ns_countries_${server}_1`)]);
...
Telegraf.prototype.handleUpdate=async function(update,...args){try{const q=update?.callback_query,cb=q?.data||'';if(q&&q.from?.id){if(cb.startsWith('ns_country_')){const p=cb.split('_');try{await this.telegram.answerCbQuery(q.id)}catch{};return renderServices(this,q,p[2],decodeURIComponent(p[3]),1)}if(cb.startsWith('ns_services_')){const p=cb.split('_');try{await this.telegram.answerCbQuery(q.id)}catch{};return renderServices(this,q,p[2],decodeURIComponent(p[3]),Number(p[4])||1)}}}catch(e){console.log('NUMBER STOCK FIX:',e.message)}return previous.call(this,update,...args)};
